...
検証状況: ユニットテスト23件 + race detector パス。gramine-direct / gramine-sgx
（`RA_TYPE=none`）で全APIの実機確認済み（2026-07-08）。
...
| `testdata/readinput.wasm` | `/data/input` 読み取りのテスト用フィクスチャ（`wasm_module/readinput/` のビルド成果物） | — |

## 2. 起動シーケンス（`main.go: main`）

1. `HOST`/`PORT` から待ち受けアドレスを決定。
2. `newStore(DATA_DIR)` — `meta/`・`blobs/` サブディレクトリを作成（0700）。
3. `newProver()` — Ed25519 鍵ペアを生成し、`/dev/attestation/*` から quote または
   ローカルレポートを取得（取得できない環境では署名のみのモードに縮退。§10）。
4. `newLifecycleManager(store, prover)` — 永続化済みメタデータを全件ロードし、
   中断状態をリカバリ（§6.4）。リカバリ失敗時は起動エラーで停止。
5. サンドボックス設定（`EXEC_TIMEOUT_SEC`, `WASM_MEM_LIMIT_PAGES`）を読み込み、
...
ルーティングは Go 1.22+ の `http.ServeMux` パターン（`POST /data/{id}/execute` 等）。
未定義パスは 404（`404 page not found`）、メソッド不一致は 405 を返す。

エラー応答は `/data` 系ではすべて JSON `{"error":"<メッセージ>"}`。
既存エンドポイント（`GET /`, `POST /execute-wasm`）は従来どおり text/plain。

### 4.1 認可（`ownerToken` / `checkToken`）

- トークンは `X-Owner-Token: <token>` または `Authorization: Bearer <token>` で渡す
  （`X-Owner-Token` 優先）。
- **登録時**にトークンが付いていた場合のみ、その ID への `execute`/`delete` に同一
  トークンを要求する（SHA-256 ハッシュを保存し、constant-time 比較）。
- トークン無しで登録されたデータは誰でも `execute`/`delete` できる（開発用の緩和。
  認可プロトコルの設計は設計書 §11 の未解決課題）。
- `status`/`proof` はトークン不要（生データ・鍵を含まず、proof は監査用のため）。
...
| `POST /execute-wasm` | 200 text（実行結果） | 400 text `WASM error: ...`（読込エラー含む） |
| `POST /data` | **201** JSON `{"data_id","registered_at"}` | 400（空ボディ）/ 413（32MiB超）/ 500 |
| `POST /data/{id}/execute` | 200 text（実行結果） | 400 / 403 / 404 / 409 / 413 / 500 |
| `DELETE /data/{id}` | 200 JSON（削除証明） | 403 / 404 / 409 / 500 |
| `GET /data/{id}/status` | 200 JSON | 404 |
| `GET /data/{id}/proof` | 200 JSON（削除証明） | 404 / 409 |

### 4.3 ライフサイクルエラー → HTTP ステータス対応
...
実行結果（execute / execute-wasm 共通、`sandbox.go: run` の整形）:
...
| REGISTERED → IN_USE | `beginExecute` | **しない**（メモリ上のみ。クラッシュ時はディスク上 REGISTERED のまま復帰） |
| IN_USE → REGISTERED | `endExecute`（ハンドラの defer で必ず呼ばれる） | しない |
...
| `owner_token_hash` | sha256(トークン) の hex（トークン付き登録時のみ） |
...
```
delete:
  1. （ロック内）検証: 存在・トークン・状態 = REGISTERED
...
| FS 制限（§8-2） | 入力データがある場合のみ、`fstest.MapFS`（**メモリ上の読み取り専用FS**）を guest の `/data` にマウント。WASM からは `/data/input` の1ファイルだけが見える。書き込みマウント無し。ステートレス実行では FS 自体を与えない。**平文がホストのディスクに書かれることはない** |
...
- `POST /execute-wasm` の入出力仕様（text/plain、`WASM error:` プレフィックス、
  出力整形）は維持。ただしタイムアウト・メモリ上限・ボディ上限 32 MiB が
  新たに適用される（従来は無制限）。
...
- **認可**: トークンは「登録時に提示されたものと同じか」のみを検証する自己申告制。
  発行・失効・利用者＝データオーナー本人の保証は未実装（設計書 §11 のとおり別途）。
...
| `TestOwnerTokenEnforcement` | トークン照合（無し/誤り → errForbidden） |
| `TestConcurrentDeleteRace` | 16並行削除で成功がちょうど1回（TOCTOU） |
| `TestPersistenceAcrossRestart` | 再起動後の状態・トークン・データ・証明の保持 |
...
| `TestAPILifecycleFlow` | HTTP 一巡 + **証明の署名を検証者の立場で検証**（§8.4 手順3–4） |
| `TestAPIRegisterValidation` / `TestAPIOwnerToken` / `TestAPIExecuteBusyConflict` / `TestAPIBodyTooLarge` | ステータスコード対応表（§4.3）の確認 |
| `TestAPIStatelessExecute` / `TestAPIHealth` | 既存エンドポイントの回帰 |
| `TestSandboxTimeout` | 無限ループが強制中断される（手組みのループWASM使用） |
| `TestSandboxMemoryLimit` | 上限超過モジュール（2000ページ要求）の拒否 |
| `TestSandboxInputMount` / `TestSandboxNoInputNoFS` | `/data/input` の可視性（入力あり時のみ） |
